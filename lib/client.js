...
//   - page visible  -> pink card + chime, auto-dismissed after 2s
//   - page hidden   -> system notification + chime
//   - final assistant message ends with ? / ？ -> "有个问题等你回答".
...
// Last assistant message body ends with ? / ？ -> the model asked a question.
...
function apply(ctx) {
  ...
  function ToastRoot() {
    ...
    React.useEffect(function () {
      ...
      var undoTimer = ctx.interval(async function () {
        if (stopped) return;
        try {
          ...
          if (data.seq > baseline) {
            ...
            if (isPageVisible()) {
              ...
              leaveTimer = ctx.timeout(function () { setLeaving(true); }, 1600);
              hideTimer = ctx.timeout(function () { setToast(null); setLeaving(false); }, 2000);
            } else {
              notifySystem(kind);
            }
          }
    ...
    var title = toast.kind === 'question'
      ? '\u6709\u4E2A\u95EE\u9898\u7B49\u4F60\u56DE\u7B54'
      : '\u4EFB\u52A1\u5B8C\u6210\u5566 \u266A';
    var sub = toast.kind === 'question'
      ? '\u56DE\u5230\u5F53\u524D\u5BF9\u8BDD\u56DE\u590D\u4E00\u4E0B\u5427'
      : '\u641E\u5B9A\uFF01\u53BB\u770B\u770B\u5427';
    return React.createElement('div', { className: 'hellokitty-done-toast' + (leaving ? ' hk-hide' : '') },
      React.createElement('span', { className: 'hk-bow' }, toast.kind === 'question' ? '\u{1F4AC}' : '\u{1F380}'),
